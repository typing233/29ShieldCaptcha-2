self.onmessage = function(e) {
    ...
    while (true) {
        const solution = counter.toString(16);
        const data = challengeId + ':' + nonce + ':' + solution;
        const hash = sha256(data);

        if (hasLeadingZeroBits(hash, target)) {
            self.postMessage({ found: true, solution: solution, iterations: counter });
            return;
        }

        counter++;
        if (counter % 10000 === 0) {
            self.postMessage({ found: false, iterations: counter });
        }
    }
};
